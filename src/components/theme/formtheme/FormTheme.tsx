...
function FormTheme() {
    ...
    if (id !== undefined) {
      try {
        ...
        if (error.toString().includes("401")) {
          ...
          ToastAlert("Erro ao atualizar o Tema!", "error");
        }
      }
    } else {
      try {
        ...
        ToastAlert("O Tema foi cadastrado com sucesso!", "sucess");
      } catch (error: any) {
        if (error.toString().includes("401")) {
          handleLogout();
        } else {
          ToastAlert("Erro ao cadastrar o Tema!", "error");
        }
      }
    }
  ...
  return (
    <main className="container flex flex-col items-center justify-center mx-auto">
      ...
      <form className="w-1/2 flex flex-col gap-4" onSubmit={createNewTheme}>
