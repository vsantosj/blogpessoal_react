...
function FormPosts() {
  ...
  async function createNewPosts(e: FormEvent<HTMLFormElement>) {
    ...
    if (id !== undefined) {
      try {
        ...
        ToastAlert("Postagem atualizada com sucesso", "sucess");
      } catch (error: any) {
        if (error.toString().includes("401")) {
          handleLogout();
        } else {
      ...
      try {
        ...
        ToastAlert("Postagem cadastrada com sucesso", "sucess");
      } catch (error: any) {
        if (error.toString().includes("401")) handleLogout();
        else ToastAlert("Erro ao cadastrar a Postagem", "error");
      }
    }
    setIsLoading(false);
    back();
  ...
  return (
    <div className="container flex flex-col mx-auto items-center">
      ...
      <form className="flex flex-col w-1/2 gap-4" onSubmit={createNewPosts}>
